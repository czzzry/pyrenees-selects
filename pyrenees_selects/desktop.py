...
class StoryboardPreparationJob:
    def __init__(self, application: Application) -> None:
        self.application = application
        self.status_path = application.paths.root / "storyboard-preparation-status.json"
        self.lock = threading.Lock()
        self.cancel_event = threading.Event()
        self.thread: threading.Thread | None = None
        self.caffeinate: subprocess.Popen[bytes] | None = None
        self._status = self._read_status()
        if self._status.get("state") == "running":
            self._status.update(state="interrupted", message="Storyboard preparation was interrupted. Resume to continue.")
            self._write_status()
    ...
    def _read_status(self) -> dict[str, Any]:
        try:
            ...
            return {"state": "idle", "message": "The two-minute storyboard is ready to prepare."}
    ...
    def status(self, project_id: str) -> dict[str, Any]:
        ...
        if status.get("project_id") != project_id and status.get("state") == "running":
            return {
                "state": "blocked",
                "project_id": project_id,
                "message": "Another project is currently preparing its storyboard. Pause it before starting this one.",
            }
        if status.get("project_id") != project_id:
            return {"state": "idle", "project_id": project_id, "message": "The two-minute storyboard is ready to prepare."}
        return status

    def start(self, project_id: str) -> dict[str, Any]:
        if self.thread and self.thread.is_alive():
            return self.status(project_id)
        if not self.application.store.project(project_id):
            raise KeyError(project_id)
        items = self.application.store.storyboard_items(project_id, 120)
        if not items:
            raise ValueError("The edit plan is not available for this project.")
        if shutil.disk_usage(self.application.paths.root).free < 1024**3:
            raise ValueError("At least 1 GB of free disk space is required for disposable storyboard previews.")
        self.cancel_event.clear()
        self._status = {
            "state": "running",
            "project_id": project_id,
            ...
            "started_at": _now(),
            "updated_at": _now(),
            "message": "Preparing the proposed ranges at 360p. Originals remain untouched.",
        }
        self._write_status()
        self.caffeinate = subprocess.Popen(
            ["/usr/bin/caffeinate", "-dimsu"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        self.thread = threading.Thread(target=self._run, args=(project_id,), name="pyrenees-storyboard", daemon=True)
        self.thread.start()
        return self.status(project_id)
    ...
    def _run(self, project_id: str) -> None:
        ...
        try:
            ...
            for index, item in enumerate(items, start=1):
                if self.cancel_event.is_set():
                    raise InterruptedError("Storyboard preparation paused.")
                self._update(current_file=item["filename"])
                try:
                    self.application.storyboard_asset(int(item["candidate_id"]))
                except Exception as exc:
                    failures.append({"filename": item["filename"], "error": str(exc)})
                self._update(processed=index, failures=failures)
            message = "The two-minute storyboard is ready for your review."
            if failures:
                message += f" {len(failures)} preview(s) need another attempt."
            self._update(state="complete", current_file="", message=message, finished_at=_now())
        except InterruptedError:
            self._update(state="interrupted", message="Storyboard preparation paused safely. Resume when ready.")
        except Exception as exc:
            self._update(state="failed", message=str(exc), finished_at=_now())
        finally:
            if self.caffeinate and self.caffeinate.poll() is None:
...
class DesktopApi:
    def __init__(self, application: Application) -> None:
        ...
        self.storyboard_preparation = StoryboardPreparationJob(application)
    ...
    def save_candidate_note(self, candidate_id: int, note: str) -> dict[str, Any]:
        return self.application.save_candidate_note(candidate_id, note)

    def refinement_state(self, project_id: str = "") -> dict[str, Any]:
        return self.application.refinement_state(project_id or None)
    ...
    def save_refinement(
        ...
        return self.application.save_refinement(candidate_id, note, note_anchor_seconds, reviewed)
    ...
    def candidate_assets(self, candidate_id: int) -> dict[str, Any]:
        ...
        return {
            ...
            "source_video_url": self.application.candidate_source(candidate_id).as_uri(),
        }

    def refinement_asset(self, candidate_id: int) -> dict[str, str]:
        video = self.application.candidate_asset(candidate_id, "video")
        return {"video_base64": base64.b64encode(video.read_bytes()).decode("ascii")}

    def storyboard_state(self, project_id: str = "", variant_seconds: int = 120) -> dict[str, Any]:
        return self.application.storyboard_state(project_id or None, int(variant_seconds))

    def hybrid_state(self, project_id: str = "") -> dict[str, Any]:
        return self.application.hybrid_state(project_id or None)
    ...
    def review_storyboard_item(
        ...
        return self.application.review_storyboard_item(storyboard_item_id, decision, replacement_candidate_id)

    def save_storyboard_note(self, storyboard_item_id: int, note: str) -> dict[str, Any]:
        return self.application.save_storyboard_note(storyboard_item_id, note)

    def review_hybrid_item(self, storyboard_item_id: int, decision: str) -> dict[str, Any]:
        return self.application.review_hybrid_item(storyboard_item_id, decision)

    def storyboard_preparation_status(self, project_id: str) -> dict[str, Any]:
        return self.storyboard_preparation.status(project_id)

    def start_storyboard_preparation(self, project_id: str) -> dict[str, Any]:
        return self.storyboard_preparation.start(project_id)

    def cancel_storyboard_preparation(self) -> dict[str, Any]:
        return self.storyboard_preparation.cancel()

    def storyboard_asset(self, candidate_id: int) -> dict[str, str]:
        video = self.application.storyboard_asset(candidate_id)
        return {"video_base64": base64.b64encode(video.read_bytes()).decode("ascii")}

    def hybrid_asset(self, candidate_id: int) -> dict[str, str]:
        video = self.application.hybrid_asset(candidate_id)
        return {"video_base64": base64.b64encode(video.read_bytes()).decode("ascii")}
